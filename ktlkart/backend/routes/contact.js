...
router.post('/send', async (req, res) => {
  ...
  const subject = type === 'reserva'
    ? `Nueva Reserva - ${product} | KTL Kart`
    : `Nueva Consulta - ${product || 'General'} | KTL Kart`;
  ...
  const html = `
    <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">
      <div style="background:#003087;padding:20px;text-align:center;border-radius:8px 8px 0 0;">
        ...
        <p style="color:#87CEEB;margin:5px 0 0;">${type === 'reserva' ? '🏎️ Nueva Reserva' : '📩 Nueva Consulta'}</p>
      </div>
      <div style="background:white;padding:25px;border-radius:0 0 8px 8px;border:1px solid #eee;">
        ...
        <p><strong>Tipo:</strong> ${type === 'reserva' ? 'Reserva' : 'Consulta'}</p>
